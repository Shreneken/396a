...
trash = []
for date_dir in os.listdir("Metacritic/metacritic_movies_json"):
    new_dir = f"Metacritic/metacritic_movies_json/{date_dir}/"
    # if int(date_dir) != 2018:
    #     continue
    try:
        for each_movie in tqdm(os.listdir(new_dir)):
            try:
                file = f"Metacritic/metacritic_movies_json/{date_dir}/{each_movie}"
